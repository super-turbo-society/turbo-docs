...
With our core structs defined and implemented, let's add some logic to delay the start of the game until the player inputs, as well as adding an in-game reset.
...
With so many moving parts in our game at this point, let's give the player some info about their status with a heads up display! 
