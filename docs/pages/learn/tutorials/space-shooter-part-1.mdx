# Space Shooter (Part 1) [Manuever in zero G, shoot the baddies.]
...
> In this two part tutorial, you will build a complex Space Shooter with enemies and powerups.
In Part 1, you will set the project up with the player, projectiles, and one enemy type.
...
- [x] Iterate through lists, updating and drawing game structs
...
> - [x] [projectile_player.wav](https://raw.githubusercontent.com/super-turbo-society/turbo-demos/refs/heads/main/space-shooter/audio/projectile_player_shoot.wav)
> - [x] [projectile_enemy.wav](https://raw.githubusercontent.com/super-turbo-society/turbo-demos/refs/heads/main/space-shooter/audio/projectile_enemy_shoot.wav)
> - [x] [projectile_hit.wav](https://raw.githubusercontent.com/super-turbo-society/turbo-demos/refs/heads/main/space-shooter/audio/projectile_hit.wav)

Add the following files to the `sprites` folder:

> Player Sprites 
>
> - [x] [player.gif](https://raw.githubusercontent.com/super-turbo-society/turbo-demos/refs/heads/main/space-shooter/sprites/player.gif)
> - [x] [player_shooting.gif](https://raw.githubusercontent.com/super-turbo-society/turbo-demos/refs/heads/main/space-shooter/sprites/player_shooting.gif)
> - [x] [projectile_player.gif](https://raw.githubusercontent.com/super-turbo-society/turbo-demos/refs/heads/main/space-shooter/sprites/projectile_player.gif)
> - [x] [projectile_player_hit.gif](https://raw.githubusercontent.com/super-turbo-society/turbo-demos/refs/heads/space-shooter/sprites/projectile_player_hit.gif)
> 
> Enemy sprites
>
...
> - [x] [projectile_enemy.gif](https://raw.githubusercontent.com/super-turbo-society/turbo-demos/refs/heads/main/space-shooter/sprites/projectile_enemy.gif)
> - [x] [projectile_enemy_hit.gif](https://raw.githubusercontent.com/super-turbo-society/turbo-demos/refs/heads/main/space-shooter/sprites/projectile_enemy_hit.gif)

We'll be adding more sprites later!
...
For now, we have no implemented structs or data we need to store and update here yet.
As we add to the project, we will come back to add new structs to the `GameState`.
We do set up a `draw()` function to keep our drawing and updating logic seperate while here.
...
### Player
Starting with the `Player` struct, we will outline some variables we will use to update and draw them. 
...
These are all the variables we will need to track the player as they move and shoot on screen. 
...
```
After saving all your changes, return to the Turbo game window to fly your player ship around!
...
:::info[Code Breakdown]
- Just like the player, `x`, `y`, and `hitbox` variables are used for movement and collisions, but also `velocity` and `angle`
...
```
Saving all your work, you now should be able to shoot projectiles in the game window! 
...
The `EnemyType` `enum` will be used to add variety to our enemies. 
For now, we will only implement the `Turret` variant.
...
- Construct a string and use it to draw a `sprite!()` 
    - We'll setup logic for using sprites based on `EnemyType` even though we're only implementing `Turret`
:::
...
impl GameState {
    ...
    fn spawn_enemies(&mut self) { // [!code focus] [!code hl] [!code ++]
        ...
        if time::tick() % spawn_rate == 0 && self.enemies.len() < 24 { // [!code focus] [!code hl] [!code ++]
            // Spawn a random enemy with these probabilities // [!code focus] [!code hl] [!code ++]
...
:::tip[Development Tip]
A little noisy, huh? We'll be adding a state manager in the next part so the enemies don't spawn and shoot without player input. 
...
- If it should collide with the player, check if it has invincibilty frames or is already at 0 HP
...
Save everything, and you're done with Part 1! Play around in the game window, shoot some enemies, and take some damage.
:::tip[Development Tip]
...
![Completed Part 1!](/space-shooter-part-1.gif)
...
If you continue to the next part, we'll add more dynamism with enemy types, powerups, and a difficulty ramp! We'll also add score tracking, main menu and gameover screens.

## Next Steps

- Continue this project with [`Space Shooter Part 2`](/learn/tutorials/space-shooter-part-2)!