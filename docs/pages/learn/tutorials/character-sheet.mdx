...
- [x] How to flip between menus using state machines
...
This will make the code we write in `state` and `player` to be recognized inside of `lib.rs` and vice versa. Don't forget to save each file with Cmd+S or Ctrl+S.
...
Additionally, if you run it from the directory you can forgo the project name
...
Now we'll add in an empty state machine. This will be how we can flip between each screen easily.
...
```
Lets add our state machine to our update loop so we can view the changes we make to it in real time
...
Try saving after every step!
:::

### Adding in our player sprite

Alright lets display our character in the middle of the screen and start creating their character screen
...
:::tip[Development Tip]
You may have noticed the player sprite is a `.aseprite` file. Turbo has unique interactions with [Aseprite](https://www.aseprite.org/) being able to call tags directly in the code. We will be taking advantage of this, feel free to read more about it [here.](https://docs.turbo.computer/learn/api/sprites)
:::
...
It's as easy to display him as this, make sure to save right after
...
If this is your first time seeing or hearing about tweens, awesome! They allow for smooth transitions of `f32` (decimal) values, they're easy to implement, and great for polish!

We need one specific `Tween` in the `GameState` for each value we want to be able to manipulate, we are going to add 8 tweens and initialize them, as well as a `Const` to the top. `Const` are just values that won't be changing
...
:::tip[Development Tip]
[`Tweens`](https://docs.turbo.computer/learn/api/tween) need an initial starting value as `f32`, an `easing`, and a duration as `usize` to initialize. Feel free to tweak the `duration` and `easing`
:::
...
Zooming in with the `camera` allows you to see our characters face and item sprites as the player looks through their inventory. It also makes the item descriptions and names larger and easier to view. Our main characters idle also has them blink, which is a nice touch while browsing the inventory system.
...
On the loadout screen itself we can use the arrowkeys to switch between sword and gun and equip it with `a`. After equipping our weapon of choice we get an animation to reflect what we have equipped as well as a blue highlight.
...
The project can always be improved, if you want to do more I suggest the following:
...
Thanks for following along and I hope it was helpful! Turboooooooooooooo

## Next Steps

- Complete the `Quest` above, post your results in the discord!
