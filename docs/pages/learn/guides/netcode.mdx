...
Like channels, getting file updates is **poll-based**. You must `watch()` files in every update loop:
...
:::warning
File watching is **not persistent**. You must call `watch()` each frame.
:::
